...
$complete_query="SELECT u.id as id, e.code, CONCAT(e.first_name,' ',e.last_name) AS full_name, u.username as username, e.work_email as email, e.contact_no as contact_no,password,u.is_active, ut.description as user_type FROM users u INNER JOIN employees e ON e.id=u.employee_id JOIN user_type ut ON ut.id=u.user_type_id {$where} {$order} {$limit}";
            // echo $complete_query;
             //var_dump($bindings);
