...
function InterviewProcess($employee_id)
{
    if (array_search($_SESSION[WEBAPP]['user']['employee_id'], $employee_id )!==false) {
        return true;
    }
    return false;
}
...
function emailer_attachment($username, $password, $from, $to, $subject, $body, $host='tls://smtp.gmail.com', $port=465)
{
    ...
    // }
    $access_id = array_column( $_SESSION[WEBAPP]['permissions'], 'user_access_id');
...
/* COMMENTED THIS BY AILEEN ROMERO 12/17/18 */
function getLate($time_in, $shifts_array, $half_day="", $for_download=0, $in_array_form=FALSE)
{
    $late="";

    if (empty($time_in) || $shifts_array['late_start']=="00:00:00" || empty($shifts_array['late_start'])) {
        return    "";
    }
    // var_dump($time_in);
...
function convertBytes($value)
{
 ...
 /* COMMENTED THIS BY AILEEN ROMERO 12/17/18 Jerwin 
function getFlexiTimeInAndOut ($time_in, $time_out, $shifts_array, $is_payroll = false) {
    if ($time_in == "" && $time_out =="") {
        return "";
    }
    ...
    if (empty($time_out) || $time_out == "00/00/0000 00:00:00") {
        $today=new DateTime();
        if ($time_in->format("Y-m-d")!==$today->format("Y-m-d")) {
            if ($is_payroll==false) {
                return array("time_in"=>$time_in->format(DATE_FORMAT_PHP." ".TIME_FORMAT_PHP), "time_out"=>"");
            } else {
                return "";
            }
        }
    ...
    $time_inY=date("Y-m-d",strtotime($time_in));
    $time_inY2=date("Y-m-d H:i:s",strtotime($time_inY." ".$shifts_array['time_in']));
    */
    /*$time_inY->format("Y-m-d")." ".$shifts_array['time_in']); 
    if ($time_inX  < $time_inY2) {
    ...
    // echo $time_out->format("Y-m-d H:i:s");
    $original_time_out = clone($time_out);

    // print_r($time_in);
    // print_r($time_out);
    if ($shifts_array['time_out']=="00:00:00") {
        $shift_out = new DateTime($time_out->format("Y-m-d")." 24:00:00");
    } else {
        $shift_out = new DateTime($time_out->format("Y-m-d")." ".$shifts_array['time_out']);
    }
    $time_out->format("Y-m-d H:i:s") ;
    $time_outY=$shift_out->format("Y-m-d H:i:s");
    if ($time_out >= $time_outY) {
        //greater than time out of the shift
    ...
    $interval->format("%d");
    if (!empty($interval)) {
        $day_hours = 24 * $interval->format("%d");
    } */

    #For Flexi TIme
    /*
    if ($interval->format("%h") + $day_hours < 9) {
        $original_interval = $original_time_out->diff($time_in);
        
        if ($original_interval->format("%h") + $day_hours > 9) {
            $hours_to_deduct = ($original_interval->format("%h")+$day_hours)-9;
        ...
        if ($interval->format("%h") + $day_hours > 9) {
            $hours_to_deduct = ($interval->format("%h") + $day_hours)-9;
    ...
    */
    // print_r($time_in->format("Y-m-d H:i:s"), $time_out->format("Y-m-d H:i:s"));
	/*
...
function getHoursWorked($time_in, $time_out, $shifts_array, $in_array_form=FALSE)
{
    ...
    if ($time_out->format("Y-m-d H:i:s") >= $shift_out->format("Y-m-d H:i:s")) {
        //greater than time out of the shift
        $time_out = new DateTime($time_out->format("Y-m-d")." ".$shifts_array['time_out']);
    }
...
function get_user_access_id_pay($user_id,$user_access_id)
{
    global $con;
    $uaid= $con->myQuery("SELECT user_id,user_access_id FROM permissions WHERE system_id=2 and user_id=?",array($user_id))->fetch(PDO::FETCH_ASSOC);
    $uid = explode("," , $uaid['user_access_id']);
    $count_uid=count($uid);
    for($x=0; $x<$count_uid; $x++){
      if($uid[$x]==$user_access_id){
...
// ADDED BY AILEEN ROMERO 11/14/2018
# this function if for checking of leave date by employee which is approved
function checkLeave1($employee_id,$date)
{
    global $con;
    $data=$con->myQuery("SELECT date_leave FROM employees_leaves_date eld INNER JOIN employees_leaves el ON eld.employees_leaves_id = el.id WHERE el.employee_id = ? AND el.leave_id <> 0 AND el.comment = '' AND el.request_status_id = 2 AND date_leave=?",array($employee_id,$date))->fetch(PDO::FETCH_ASSOC);
    return $data;
}
// END OF ADDED BY AILEEN ROMERO 11/14/2018
...
function check_loans($employee_id)
{
    ...
    $data=$con->myQuery("SELECT emp_loan_id,employee_id FROM emp_loans WHERE status_id = 1 AND employee_id = ?",array($employee_id))->fetchAll(PDO::FETCH_ASSOC);
    return $data;
}
...
function get_loan_details($employee_id,$emp_loan_id)
{
    global $con;
    ...
    $data=$con->myQuery("SELECT
        ...
        FROM
        emp_loans
        WHERE status_id = 1 AND employee_id = ? AND emp_loan_id =?",array($employee_id,$emp_loan_id))->fetch(PDO::FETCH_ASSOC);
    return $data;
...
function get_deminimis($emp_id)
{
    ...
    $deminimis= $con->myQuery("SELECT employees.`code`,
        ...
        SUM(de_minimis_benefits.dmb_amount) as 'dmb_amount',
...
function get_receivablesdeduction($emp_id)
{
    ...
    $receivablesdeduction= $con->myQuery("SELECT employees.`code`,
        ...
        SUM(receivable_and_taxable_allowances.rta_amount) as 'rta_amount',
...
function get_company_deductions($emp_id)
{
    ...
    $company_deduction= $con->myQuery("SELECT cd.comde_code,
        SUM(ecd.emp_comde_amt) as 'emp_comde_amt',
...
// {
//     global $con;
//     $payroll_adjustments= $con->myQuery("SELECT employee_id,SUM(amount) as amount, adjustment_type, taxable_non_taxable
...
// }
function get_payroll_adjustments_add_tax($emp_id,$date_from,$date_to)
{
    global $con;
    $payroll_adjustments= $con->myQuery("SELECT employee_id, sum(amount) as 'amount'
        FROM payroll_adjustments 
        WHERE 
        adjustment_type = 1 AND taxable_non_taxable = 1 AND
        employee_id = ? AND
        status = '0'",array($emp_id))->fetch(PDO::FETCH_ASSOC);

    return $payroll_adjustments['amount'];
}

function get_payroll_adjustments_less_tax($emp_id,$date_from,$date_to)
{
    global $con;
    $payroll_adjustments= $con->myQuery("SELECT employee_id,sum(amount) as 'amount'
        FROM payroll_adjustments 
        WHERE 
        adjustment_type = 0 AND taxable_non_taxable = 1 AND
        employee_id = ? AND
        status = '0'",array($emp_id))->fetch(PDO::FETCH_ASSOC);
...
function get_payroll_adjustments_add_nontax($emp_id,$date_from,$date_to)
{
    global $con;
    $payroll_adjustments= $con->myQuery("SELECT employee_id,sum(amount) as 'amount'
        FROM payroll_adjustments 
        WHERE 
        adjustment_type = 1 AND taxable_non_taxable = 2 AND
        employee_id = ? AND
        status = '0'",array($emp_id))->fetch(PDO::FETCH_ASSOC);
...
function get_payroll_adjustments_less_nontax($emp_id,$date_from,$date_to)
{
    global $con;
    $payroll_adjustments= $con->myQuery("SELECT employee_id,sum(amount) as 'amount'
        FROM payroll_adjustments 
        WHERE 
        adjustment_type = 0 AND taxable_non_taxable = 2 AND
        employee_id = ? AND
        status = '0'",array($emp_id))->fetch(PDO::FETCH_ASSOC);
...
function get_employees_leaves_halfday_without_pay($emp_id, $date_from,$date_to) //half day without pay --> leave_id = 0 AND `comment` <> ''
{
    global $con;
    $leaves= $con->myQuery("SELECT * FROM
        employees_leaves
        INNER JOIN `leaves` ON employees_leaves.leave_id = `leaves`.id
        WHERE request_status_id = '2' AND
        employee_id = ? AND
        DATE_FORMAT(date_start, '%Y-%m-%d') >= ? AND
        DATE_FORMAT(date_end, '%Y-%m-%d') <= ? AND
        `leaves`.is_pay = 0 AND `comment`<>''",array($emp_id,$date_from,$date_to))->fetchAll(PDO::FETCH_ASSOC);
    return count($leaves);
}

function get_employees_leaves_wholeday_without_pay($emp_id, $date_from,$date_to) 
{
    global $con;
    $leaves= $con->myQuery("SELECT * FROM
        employees_leaves
...
function get_employees_leaves_halfday_with_pay($emp_id, $date_from,$date_to) 
{
    global $con;
    $leaves= $con->myQuery("SELECT * FROM
        employees_leaves
        ...
        DATE_FORMAT(date_start, '%Y-%m-%d') >= ? AND
        DATE_FORMAT(date_end, '%Y-%m-%d') <= ? AND
        `leaves`.is_pay = 1 AND `comment`<>''",array($emp_id,$date_from,$date_to))->fetchAll(PDO::FETCH_ASSOC);
    return count($leaves);
}

function get_employees_leaves_wholeday_with_pay($emp_id, $date_from,$date_to) //wholeday with pay --> leave_id > 0 AND `comment` = ''
{
    global $con;
    $leaves= $con->myQuery("SELECT * FROM
        employees_leaves
...
function get_ecola($emp_id)
{
    global $con;
    $data=$con->myQuery("SELECT ecola FROM employees WHERE is_deleted = 0 AND id = ?",array($emp_id))->fetch(PDO::FETCH_ASSOC);
...
{
    global $con;
    $ots=$con->myQuery("SELECT employees_id,ot_date,SUM(no_hours) as 'no_hours' FROM employees_ot WHERE employees_id=? AND ot_date=? AND request_status_id='2'", array($employee_id,$ot_date))->fetch(PDO::FETCH_ASSOC);
...
function getOvertimePerDayForPayroll2($employee_id, $ot_date)
{
    global $con;
    $ots=$con->myQuery("SELECT employees_id,ot_date,SUM(no_hours) as 'no_hours' FROM employees_ot WHERE employees_id=? AND ot_date=? AND request_status_id='2'", array($employee_id,$ot_date))->fetch(PDO::FETCH_ASSOC);
    // echo '<pre>';
...
{
    global $con;
    return $con->myQuery("SELECT is_team_lead,is_manager FROM projects_employees WHERE employee_id=? AND project_id=?", array($employee_id,$project_id))->fetch(PDO::FETCH_ASSOC);
}

function getFiledOvertime($time_from, $time_to, $employee_id)
{
    global $con;
    ...
    return $con->myQuery("SELECT
                    ...
                    (
                        (
                            :time_from
                            BETWEEN 
                            STR_TO_DATE(CONCAT(ot_date , ' ' , time_from), '%Y-%m-%d %H:%i:%s')
                            AND
                            STR_TO_DATE(CONCAT(ot_end_date , ' ' , time_to), '%Y-%m-%d %H:%i:%s')
                            OR
                            :time_to
                            BETWEEN 
                            STR_TO_DATE(CONCAT(ot_date , ' ' , time_from), '%Y-%m-%d %H:%i:%s')
                    ...
                    AND request_status_id IN (1, 2, 3)", array("time_from"=>$time_from, "time_to"=>$time_to, "employee_id"=> $employee_id))->fetchColumn();
}

#JP
function getApprovedOvertimePerDay($date, $employee_id)
{
    global $con;
    ...
    return $con->myQuery("SELECT
                    ...
                    AND ot_date = :ot_date
                    AND request_status_id IN (2) ORDER BY 
                    ot_date,time_from", array("ot_date"=>$date, "employee_id"=> $employee_id))->fetchAll(PDO::FETCH_ASSOC);
}
#ENDJP
...
$con=new myPDO('db_spark_hrms', 'spark_hrms_admin','P@ssw0rd');
