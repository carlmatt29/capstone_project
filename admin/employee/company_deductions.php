<?php
  ...
  $data=$con->myQuery("SELECT ecd.id,ecd.emp_code,ecd.comde_code,ecd.emp_comde_amt,ecd.emp_comde_start_date,ecd.emp_comde_end_date,ecd.emp_deduct_type,cd.comde_desc,ecd.payslip FROM employee_company_deductions ecd JOIN company_deductions cd ON cd.comde_code=ecd.comde_code WHERE ecd.is_deleted=0 AND ecd.emp_id=?", array($employee['id']))->fetchAll(PDO::FETCH_ASSOC);
  if (!empty($_GET['ec_id'])) {
...
<div id='collapseForm' class='collapse'>
  <form class='form-horizontal' action='save_employee_company_deductions.php' method="POST" >
    ...
    <input type='hidden' name='id' value='<?php echo !empty($record)?$record['id']:''; ?>'>
      ...
      <div class="form-group">
        <label for="supervisor_id" class="col-md-3 control-label">Payslip *</label>
        <div class="col-md-7">
                <select class='form-control cbo' id='payslip' name="payslip[]" data-placeholder="Select payslip" style='width:100%;' multiple="multiple" required>
                <option></option>
                <option value ="Payslip 1">Payslip 1</option>
                <option value ="Payslip 2">Payslip 2</option>
            </select>
        </div>
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
        <div class="col-sm-10 col-md-offset-2 text-center">
          <a href='frm_employee.php?id=<?php echo $employee['id']?>&tab=<?php echo $tab?>' class='btn btn-default' onclick="return confirm('<?php echo empty($record)?"Cancel creation of new company deduction?":"cancel modification of company deduction?" ?>')">Cancel</a>
...
<form class='form-horizontal' action='' method="POST" target="_blank">
  <div class='text-left'>
<button class='btn btn-warning' name="pdf" >Download as PDF </button>
</div>
</form>
<br/>
<table id='ResultTable' class='table table-bordered table-striped'>
  ...
  <tbody>
    ...
    ?>
      <tr>
        <td class='text-center'><?php echo htmlspecialchars($row['payslip'])?></td>
