...
<div id='collapseForm' class='collapse'>
    <form class='form-horizontal' action='save_emp_certifications.php' method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <div class="col-md-7">
                <select name='certification_id' class='form-control cbo' data-placeholder="Select Certification" <?php echo !(empty($record))?"data-selected='".$record['certification_id']."'":NULL ?> style='width:100%' required>
                    <?php echo makeOptions($certifications); ?>
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            <label class="col-sm-3 control-label">Upload Certificate<br/> <small>Upload Limit: <?php echo ini_get('upload_max_filesize')."B";?> </small></label>
            <div class="col-sm-7">
                <input type='file' name='file' class="filestyle" data-classButton="" data-buttonName="btn btn-flat btn-default" data-input="true" data-classIcon="icon-plus" data-buttonText=" &nbsp;Select File" data-buttonBefore='true' accept="image/*" >
        ...
        <?php foreach($data as $row): ?>
            <tr>
                ...
                <td class='text-center'>
                    <a href='download_file.php?id=<?php echo $row['id']?>&type=cert' class='btn btn-default'><span class='fa fa-download'></span></a>
                    <?php 
                        $file=$con->myQuery("SELECT file_name,file_location FROM employees_certifications WHERE id=? AND is_deleted=0",array($row['id']))->fetch(PDO::FETCH_ASSOC); 
                    ?>
                    <a href="img_certifications/<?php echo $file['file_location']; ?>" class='btn btn-info btn-sm' title='View File' target="_blank"><span class='fa fa-search'></span></a>
...
<?php  if($has_error===TRUE || !empty($record)): ?>
